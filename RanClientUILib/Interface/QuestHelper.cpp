...
bool  CQuestHelper::UpdateQuestInfo ( DWORD dwQuestID )
{
	...
	m_pQuestText->AddTextNoSplit( strTitle, NS_UITEXTCOLOR::LIGHTSKYBLUE );
	...
	if ( pQuestStep->IsNEED_QITEM() )
	{
		...
		for ( int i = 0 ; i < pQuestStep->m_vecMOBGEN_QITEM.size(); ++i )
		{
			...
			m_pQuestText->AddTextNoSplit( strItem, NS_UITEXTCOLOR::LIGHTSKYBLUE );
	...
	{		
		...
		m_pQuestText->AddTextNoSplit( strMob, NS_UITEXTCOLOR::LIGHTSKYBLUE );
	}
