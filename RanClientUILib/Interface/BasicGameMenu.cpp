			...
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				if ( dwMsg & UIMSG_LB_UP )
				{
						...
						&&  !CInnerInterface::GetInstance().IsVisibleGroup ( TRADEINVENTORY_WINDOW ) )
					{
						...
						CInnerInterface::GetInstance().GetInventoryWindow()->INVENTORY();
					}
					else
					{
						int nSTATE = CInnerInterface::GetInstance().GetInventoryWindow()->GetSTATE();
						if ( nSTATE == 0 ) CInnerInterface::GetInstance().HideGroup ( INVENTORY_WINDOW );
						else if ( nSTATE == 1 ) CInnerInterface::GetInstance().GetInventoryWindow()->INVENTORY();
					}
				}
			}
		}
