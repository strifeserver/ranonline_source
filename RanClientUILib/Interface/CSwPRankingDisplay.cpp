...
void CSwPRankingDisplay::RefreshSwPRanking()
{
	...
	for ( int i = 0; i < nRankNum; ++i )
	{
		...
		m_pSwPNameText->AddText( strName, NS_UITEXTCOLOR::LIGHTGREY );

		strKill.Format( "%d" ,vecSwPRank[i].wKillNum);
		m_pSwPKillText->AddText( strKill, NS_UITEXTCOLOR::LIGHTGREY );

		strDeath.Format( "%d" ,vecSwPRank[i].wDeathNum);
		m_pSwPDeathText->AddText( strDeath, NS_UITEXTCOLOR::LIGHTGREY );
	}
	...
	for ( int i = 0; i < nRankNum1; ++i )
	{
		...
		m_pSwSchoolText->AddText( strSchool1, NS_UITEXTCOLOR::LIGHTGREY );

		strKill1.Format( "%d" ,vecSwRank[i].wKillNum);
		m_pSwKillText->AddText( strKill1, NS_UITEXTCOLOR::LIGHTGREY );

		strDeath1.Format( "%d" ,vecSwRank[i].wDeathNum);
		m_pSwDeathText->AddText( strDeath1, NS_UITEXTCOLOR::LIGHTGREY );
	}
}