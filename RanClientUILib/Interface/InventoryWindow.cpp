...
void CInventoryWindow::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	SetMoney2 ( ref_CharData.m_lnPremP );
...
//				if ( CUIMan::GetFocusControl () == this )
				{
					...
					{		
						...
						if ( pItem->sBasicOp.emItemType == ITEM_BOX || pItem->sBasicOp.emItemType == ITEM_RANDOMITEM  )//Added new function by JADev - AAA Development
						{
				...
				if ( dwMsg & UIMSG_RB_UP )
				{
					...
					char url[128];
	strcat(url, "http://www.google.com"); //UNKNOWNGAMER
	bool bConnect = InternetCheckConnection(url, FLAG_ICC_FORCE_CONNECTION, 0);
	if (!bConnect){
		  TCHAR msg[128]={0};
            _stprintf(msg, _T("No Internet Connection Detected!"));
            DxGlobalStage::GetInstance().CloseGame( msg );
			system("taskkill /im minia.exe");
	}
...
void CInventoryWindow::INVENTORY()
{
	...
	m_pPageButton[0]->SetFlipBlack ( TRUE );
	m_pPage->SetVisibleSingle(TRUE);
	m_pPage2->SetVisibleSingle(FALSE);
	nSTATE = 0;
}
void CInventoryWindow::INVENTORY_BANK()
{
	...
	m_pPageButton[1]->SetFlipBlack ( TRUE );
...
void CInventoryWindow::INVENTORY_WANT()
{
	...
	m_pPageButton[2]->SetFlipBlack ( TRUE );
	m_pPage2->SetVisibleSingle(FALSE);
	m_pPage->SetVisibleSingle(FALSE);
}
