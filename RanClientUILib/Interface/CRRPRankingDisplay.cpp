...
void CRRPRankingDisplay::RefreshRrPRanking()
{
	...
	for ( int i = 0; i < nRankNum; ++i )
	{
		...
		m_pRrPNameText->AddText( strName, NS_UITEXTCOLOR::LIGHTGREY );

		strKill.Format( "%d" ,vecRrPRank[i].wKillNum);
		m_pRrPKillText->AddText( strKill, NS_UITEXTCOLOR::LIGHTGREY );

		strDeath.Format( "%d" ,vecRrPRank[i].wDeathNum);
		m_pRrPDeathText->AddText( strDeath, NS_UITEXTCOLOR::LIGHTGREY );
	}
}