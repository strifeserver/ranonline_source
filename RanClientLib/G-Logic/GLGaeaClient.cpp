...
HRESULT GLGaeaClient::FrameMove ( float fTime, float fElapsedTime )
{
	...
	if ( m_bSchoolWars )
	{
		...
		if ( m_fAge > 1.0f ) CInnerInterface::GetInstance().UpdateClubBattleTime( m_fSchoolWarsTimer );
	}
	...
	if ( m_bTowerWars )
	{
		...
		if ( m_fAge > 1.0f ) CInnerInterface::GetInstance().UpdateClubBattleTime( m_fTowerWarsTimer );
	}
...
void GLGaeaClient::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_TYRANNY_PARTICIPATE:
		{
		   ...
		   if ( pNetMsg->bParticipate )
		   {
				if ( !IsTowerWars() )
				{
					if ( !m_bParticipate )
					{
		CInnerInterface::GetInstance().PrintConsoleText ( "You are now queued from Tyranny." );
	...
	case NET_MSG_GM_VIEWALLPLAYER_FLD_FB:
		...
		case NET_MSG_GCTRL_VIEW_PROPERTIES_FB://�d���ݩ�
		{
			m_Character.MsgProcess ( nmg );
		}
		break;
	...
	case NET_MSG_GCTRL_SCHOOL_WARS_START_BRD:
		{
			...
			if ( pNetMsg->nTIME==0 )
			{
				//CInnerInterface::GetInstance().SchoolWarStarted( nmg );
	...
	case NET_MSG_GCTRL_ROYAL_RUMBLE_END_BRD:
		{
			//fix announcement on RR by CNDev
			GLMSG::SNET_ROYAL_RUMBLE_END_BRD *pNetMsg = (GLMSG::SNET_ROYAL_RUMBLE_END_BRD *)nmg;
			CInnerInterface::GetInstance().PrintConsoleText ( "%s Royal Rumble has ended", pNetMsg->szName);
	...
	case NET_MSG_GCTRL_TOWERWARS_START_BRD:
		{
			...
			if ( pNetMsg->nTIME==0 )
			{
				...
				if ( m_bParticipate )
				{
					...
					//CInnerInterface::GetInstance().SchoolWarStarted ();
	...
	case NET_MSG_GCTRL_TOWERWARS_END_BRD:
		{
			...
			GLMSG::SNET_TOWERWARS_PREVIOUS_RECORD NetMsg;
			NetMsg.pTWRecord.fElapsedTime = m_fTWDurationTime;
			NetMsg.pTWRecord.wSchool = m_wSwSchool;
			NETSENDTOFIELD ( &NetMsg );

			m_pTWRecord.fElapsedTime = m_fTWDurationTime;
			m_pTWRecord.wSchool = m_wSwSchool;

			if(m_vecCtfRank.size() != 0)
			{
				m_pTWRecord.strName = m_vecCtfRank[0].szCharName;
			}
			else
			{
				m_pTWRecord.strName = "";
	...
	case NET_MSG_GCTRL_TOWERWARS_CTFRANKING_UPDATE:
		{
			...
			for ( int i = 0; i < nRankNum; ++i )
			{
				...
				if ( nIndex < nSize )
				{
					m_vecCtfRank[nIndex] = pNetMsg->sSwPRank[i];
				}
				else
				{
	...
	case NET_MSG_GCTRL_TOWERWARS_PREVIOUS_RECORD_FLD:
		{
			GLMSG::SNET_TOWERWARS_PREVIOUS_RECORD_FLD* pNetMsg = ( GLMSG::SNET_TOWERWARS_PREVIOUS_RECORD_FLD* )nmg;
			m_pTWRecord.fElapsedTime = pNetMsg->pTWRecord.fElapsedTime;
			m_pTWRecord.wSchool = pNetMsg->pTWRecord.wSchool;
			m_pTWRecord.strName = pNetMsg->pTWRecord.strName;
		}
		break;
	...
	case NET_MSG_SERVERTIME_BRD:
		...
		break;	
	// Detect WPE/RPE buff  Global Announcement - JAYDev
...
/*HRESULT GLGaeaClient::ReqSchoolWarsEnd()
{
		GLMSG::SNET_SCHOOL_WARS_REMAIN_BRD NetMsgBrd;
		NetMsgBrd.dwTime = 60.0;
		GLAgentServer::GetInstance().SENDTOALLCLIENT ( &NetMsgBrd );
		NETSENDTOFIELD ( &NetMsgBrd );

		return TRUE;
}*/
HRESULT GLGaeaClient::ReqSchoolWarsEnd()
{
	GLSchoolWarsAgentMan::GetInstance().SetRemainTimeEnd();
