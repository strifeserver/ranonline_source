...
namespace GLMSG
{
	...
	struct SNET_SW_TOWERDOWN_SG
	{
		NET_MSG_GENERIC		nmg;
		bool		dwSGD;

		SNET_SW_TOWERDOWN_SG()
			: dwSGD (false)
		{
			nmg.dwSize = sizeof(SNET_SW_TOWERDOWN_SG);
			nmg.nType = NET_MSG_GCTRL_TOWERDOWN_SG;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	struct SNET_SW_TOWERDOWN_SG_BRD
	{
		NET_MSG_GENERIC		nmg;
		bool		dwSGD;

		SNET_SW_TOWERDOWN_SG_BRD()
			: dwSGD (false)
		{
			nmg.dwSize = sizeof(SNET_SW_TOWERDOWN_SG_BRD);
			nmg.nType = NET_MSG_GCTRL_TOWERDOWN_SG_BRD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNET_SW_TOWERDOWN_MP
	{
		NET_MSG_GENERIC		nmg;
		bool		dwMPD;

		SNET_SW_TOWERDOWN_MP()
			: dwMPD (false)
		{
			nmg.dwSize = sizeof(SNET_SW_TOWERDOWN_MP);
			nmg.nType = NET_MSG_GCTRL_TOWERDOWN_MP;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	
	struct SNET_SW_TOWERDOWN_MP_BRD
	{
		NET_MSG_GENERIC		nmg;
		bool		dwMPD;

		SNET_SW_TOWERDOWN_MP_BRD()
			: dwMPD (false)
		{
			nmg.dwSize = sizeof(SNET_SW_TOWERDOWN_MP_BRD);
			nmg.nType = NET_MSG_GCTRL_TOWERDOWN_MP_BRD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
