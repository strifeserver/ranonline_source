...
BOOL GLAgentServer::MsgReqFieldSvrCharChkFb ( NET_MSG_GENERIC* nmg, DWORD _dwClientID, DWORD _dwGaeaID )
{
	...
	if ( pLAND )
	{
		...
		{
			GLSchoolWars* pSW = GLSchoolWarsAgentMan::GetInstance().Find ( pLAND->m_dwSchoolWarMapID );
		...
		if ( pLAND->m_bRoyalRumbleMap )
		{
			...
			if ( pRR )
			{
				sGenMapID = pRR->m_dwClubHallMap;
				dwGenGateID = pRR->m_dwClubHallGate;
			}
		}
	}
...
BOOL GLAgentServer::MsgReqGateOut ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	if ( pNextLandMan->m_bTowerWarsMap || pNextLandMan->m_bRoyalRumbleMap )
	{
		if ( GLCONST_CHAR::bPKLESS || pChar->m_nChannel != 0) 
		{
			GLMSG::SNETREQ_GATEOUT_FB NetMsgFB;
...
BOOL GLAgentServer::MsgCTFParticipate ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	GLMSG::SNET_TYRANNY_PARTICIPATE NetMsgBrd;
...
//add tower down
BOOL GLAgentServer::MsgTowerDownPHX ( NET_MSG_GENERIC* nmg )
{
		GLMSG::SNET_SW_TOWERDOWN *NetMsgBrd = (GLMSG::SNET_SW_TOWERDOWN *) nmg;
		GLMSG::SNET_SW_TOWERDOWN_BRD NetMsg;
		
		NetMsg.bOn = NetMsgBrd->bOn;
		NetMsg.emFB = NetMsgBrd->emFB;
		NetMsg.emSchool = NetMsgBrd->emSchool;
...
HRESULT GLAgentServer::MsgProcess ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	case NET_MSG_WPE_DT_BRD:		                	MsgWPEDetectedBrd ( nmg ); break; // wpe detection by JAYDev
	...
	case NET_MSG_GCTRL_TOWERDOWN:					MsgTowerDownPHX	( nmg );		break;
	...
	case NET_MSG_GCTRL_REQ_SERVER_ROYAL_RUMBLE_REMAIN_AG:	MsgRoyalRumbleRemainTime (nmg );
