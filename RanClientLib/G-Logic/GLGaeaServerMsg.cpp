...
BOOL GLGaeaServer::TowerDownPHX ( GLMSG::SNET_SW_TOWERDOWN_BRD *pNetMsg )
{
	SNATIVEID nidMAP( 22 , 0 );
	GLLandMan *pLandMan = GetByMapID( nidMAP );
	if( pLandMan )
	{
		GLMSG::SNET_SW_TOWERDOWN_BRD *NetMsg = (GLMSG::SNET_SW_TOWERDOWN_BRD *) pNetMsg;
		NetMsg->bOn = pNetMsg->bOn; 
		NetMsg->emFB = pNetMsg->emFB;
		NetMsg->emSchool = pNetMsg->emSchool;
		GLCHARNODE* pCharNode = pLandMan->m_GlobPCList.m_pHead;
		for ( ; pCharNode; pCharNode = pCharNode->pNext )
		{
...
BOOL GLGaeaServer::RequestInvenRecall ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_INVEN_RECALL *pNetMsg )
{
	...
	if ( !pLandMan )					return FALSE;
	if ( !pLandMan->IsCardActivity () || pItem->sDrugOp.emDrug==ITEM_DRUG_CALL_SCHOOL && pItem->sDrugOp.emDrug==ITEM_DRUG_CALL_REGEN && pItem->sDrugOp.emDrug==ITEM_DRUG_CALL_LASTCALL )
	{
		NetMsgFB.emFB = EMREQ_RECALL_CARD_ITEM;// Added by JADev - Anti SP, BP During War
		SENDTOCLIENT ( dwClientID, &NetMsgFB );
		return FALSE;
	}
	SNATIVEID sMAPID(false);
...
HRESULT GLGaeaServer::MsgProcess ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	case NET_MSG_GCTRL_TOWERDOWN_BRD:
		{
			TowerDownPHX ( (GLMSG::SNET_SW_TOWERDOWN_BRD *) nmg );
		}
	...
	case NET_MSG_GCTRL_TOWERWARS_CTFICON_REQ:
		...
		break;
	case NET_MSG_GCTRL_TOWERWARS_PREVIOUS_RECORD:
		ReqTWRecordPreviousResult ( dwClientID, dwGaeaID, (GLMSG::SNET_TOWERWARS_PREVIOUS_RECORD*) nmg );
		break;
//----------------------------------------------------------------------------------------------------------------
